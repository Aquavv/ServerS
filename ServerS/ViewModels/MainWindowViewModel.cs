...
namespace ServerPickerX.ViewModels
{
    public partial class MainWindowViewModel : ViewModelBase
    {
        ...
        [ObservableProperty]
        public bool isMasterToggleEnabled = true;

        partial void OnIsMasterToggleEnabledChanged(bool value)
        {
        ...
        {
            if (ServerModels.Count == 0) return false;

            var toBlock = new ObservableCollection<ServerModel>(ServerModels.Where(s => !s.IsSelected));
            var toUnblock = new ObservableCollection<ServerModel>(ServerModels.Where(s => s.IsSelected));

            bool success = true;
            if (toBlock.Count > 0)
                success &= await PerformOperationAsync(true, toBlock, false);
            
            if (toUnblock.Count > 0)
                success &= await PerformOperationAsync(false, toUnblock, false);
            ...
            if (success)
            {
                await _messageBoxService.ShowMessageBoxAsync(
                    "Success",
                    $"Server selection applied successfully to Firewall!\n\nBlocked: {toBlock.Count} servers\nAllowed: {toUnblock.Count} servers",
                    Icon.Success
                );
            }
